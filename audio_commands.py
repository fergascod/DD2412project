...
M = 3
l2_reg = 3e-4
tr_batch_size = int(batch_size / batch_repetitions)

def main():
    test_batch_size = int(batch_size)
    ds_info = tfds.builder('speech_commands').info
    train_ds=tfds.load('speech_commands', split='train', shuffle_files=True, batch_size=tr_batch_size).cache().shuffle(1000).prefetch(buffer_size=AUTO)
    test_ds = tfds.load('speech_commands', split='test', shuffle_files=True, batch_size=test_batch_size).cache().shuffle(1000).prefetch(buffer_size=AUTO)
    training_size = ds_info.splits['train'].num_examples
    test_size = ds_info.splits['test'].num_examples
    input_shape = list(ds_info.features['image'].shape)
    classes = list(ds_info.features['label'].shape[1])
    # WRN params
    n, k = 28, 10
    lr_decay_ratio = 0.2
    base_lr = 0.1 * tr_batch_size // 128
    lr_warmup_epochs = 1
    lr_decay_epochs = [80, 160, 180]
    EPOCHS = 250
    steps_per_epoch = training_size // tr_batch_size
    steps_per_eval = test_size // test_batch_size
    ...
    for epoch in range(0, EPOCHS):
        ...
        t1 = time.time()
        train(train_ds, model, optimizer, training_metrics, classes)
        t2 = time.time()
        if (epoch + 1) % 50 == 0:
        ...
        t3 = time.time()
        compute_test_metrics(model, test_ds, test_metrics, classes)
        t4 = time.time()
        test_metric = {}
